...
class Lab5():
    def __init__(self, p, xlist, rnd):
        ...
        self.l = sqrt(
            sqrt(2 ** (k - 2) * (2 ** k + 2 * k + 1)) - 2 ** (k - 1))  # Значення плеча для k експериментів ПФЕ
    ...
    def equation(self, N, m):
        ...
        if N != 4:  # N = 8,15
            ...
            if N == 15:
                xmatr.extend([[-l, 0, 0], [l, 0, 0], [0, -l, 0], [0, l, 0], [0, 0, -l], [0, 0, l], [0, 0, 0]])
                ...
                for i in range(N):
                    xvarlist = (1, (xmatr[i][0]), (xmatr[i][1]), (xmatr[i][2]), (xmatr[i][0] * xmatr[i][1]),
                                ...
                                (xmatr[i][0] * xmatr[i][1] * xmatr[i][2]), xmatr[i][0] * xmatr[i][0],
                                xmatr[i][1] * xmatr[i][1], xmatr[i][2] * xmatr[i][2])
                    for j in range(len(matrmlist)):
                        for k in range(len(matrmlist)):
                            matrmlist[j][k] += xvarlist[j] * xvarlist[k]
                        matrklist[j] += ymed[i] * xvarlist[j]
                solve = linalg.solve(array(matrmlist), array(matrklist))  # Коефіціенти bi норм. N = 15
                for i in solve:
            ...
            for i in xmatr:
                ...
                for j in i:
                    ...
                    if j == 1:
                        ...
                        i[k] = i[k] * (self.xlist[k][1] - self.xlist[k][0]) / 2 + self.x0[k]
            ...
            for i in range(N):
                ...
                for j in range(len(matrmlist)):
                    for k in range(len(matrmlist)):
                        matrmlist[j][k] += xvarlist[j] * xvarlist[k]
                    matrklist[j] += ymed[i] * xvarlist[j]
            solve = linalg.solve(array(matrmlist), array(matrklist))  # Коефіціенти bi нaтур. N = 8,15
